...
struct LinearProber : public Prober<KeyType> {
    ...
    HASH_INDEX_T next() 
    {
        ...
        if(this->numProbes_ >= this->m_) {  // DONE
            return this->npos; 
        }
...
private:
    ...
    int numKeys;  // number of non-deleted keys
    int numInserts;
       ...
       :  hash_(hash), kequal_(kequal), prober_(prober)
{
    ...
    mIndex_ = 0;
    totalProbes_ = 0;
    numKeys = 0;  // number of non-deleted keys
    numInserts = 0;
...
void HashTable<K,V,Prober,Hash,KEqual>::insert(const ItemType& p){
    // HASH_INDEX_T loc = this->hash_(p.first);  // get location from hash

    double alpha = (double)numInserts/CAPACITIES[mIndex_]; 
    if(alpha >= resizeAlpha_){  // resize table
        resize();
    }
    
    HASH_INDEX_T loc = this->probe(p.first);

    if(loc == npos){  // no free spot
...
void HashTable<K,V,Prober,Hash,KEqual>::resize(){
    std::vector<HashItem*> table = table_;  // copy of table_
    table_.assign(CAPACITIES[++mIndex_], nullptr);  // resize table_
    numKeys = 0;
    numInserts = 0;

    for(size_t i = 0; i < table.size(); i++){  // insert non-deleted items to table_
...
HASH_INDEX_T HashTable<K,V,Prober,Hash,KEqual>::probe(const KeyType& key) const{
    ...
    while(Prober::npos != loc)
    {
        ...
        else if(kequal_(key, table_[loc]->item.first) and table_[loc]->deleted == false) {  // DONE
            return loc;
        }
        loc = prober_.next();
        totalProbes_++;
