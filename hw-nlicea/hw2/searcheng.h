...
private:
    ...
    //    - the webpages that match a particular search term - map(string, WebPageSet)
    std::map<std::string, WebPageSet> searchTerms_;
    //    - Webpage objects and ability to look them up via
    //       their page/filename map(string, webpage pointers)
    std::map<std::string, WebPage*> pageObjects_;
};
